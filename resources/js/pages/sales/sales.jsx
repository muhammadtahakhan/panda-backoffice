...
import { deleteSaleOrders, fetchSaleOrders, hideForm, showForm } from '../../redux/saleOrderSlice';
...
export default function Sales() {
    ...
    const { listingData, isLoading, formVisibility, meta } = useSelector(state => state.saleOrders);
    ...
    useEffect(() => {
      ...
      // =================================================================================Data Table Template
    const paginatorLeft = <Button  onClick={()=>{dispatch( fetchSaleOrders() )} } type="button" icon="pi pi-refresh" text />;
    ...
    const editBodyTemplate = (rowData) => {
        return (
            <>
                <TieredMenu onHide={()=>{ setSelectedRow({})  }} model={items} popup ref={menu} breakpoint="767px" />
                <i className="custom-target-icon pi pi-ellipsis-v p-text-secondary p-overlay-badge" onClick={(e) => {setSelectedRow({rowData}); menu.current.toggle(e)}} ></i>
            </>);
        ...
        <>
            ...
            {isLoading &&
             ...
             <div className="bg-white border border-gray-200 rounded-lg shadow h-max px-2 py-2">
                <DataTable scrollable header={header} value={listingData} tableStyle={{ minWidth: '50rem' }}>
                    <Column frozen field="customer.name" header="Name" style={{ width: '25%' }}></Column>
                    <Column field="customer.name_urdu" header="Name in Urdu" style={{ width: '25%' }}></Column>
                    <Column field="order_date" header="Date" style={{ width: '25%' }}></Column>
                    <Column field="comment" header="Comment" style={{ width: '25%' }}></Column>
                    <Column field="total_amount" header="Order Total"></Column>
                    <Column header="Actions" body={editBodyTemplate} style={{ width: '20%' }}></Column>
